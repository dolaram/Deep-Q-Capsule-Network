...
def TrainNetwork4FlappyBird(s, coeff, readout, sess): 
    tick = time.time()
    ...
    x_t = cv2.cvtColor(cv2.resize(x_t[:,:-110,:], (28, 28)), cv2.COLOR_BGR2GRAY)
    ...
    while True:
        ...
        x_t1 = cv2.cvtColor(cv2.resize(x_t1_colored[:,:-110,:], (28, 28)), cv2.COLOR_BGR2GRAY)
        ...
        if(r_t==1):
            pscore += 1
            print("ts", t,"Q_MAX %e" % Q_MAX," pscore ",pscore," loss ",loss," Q_MAX %e" % np.max(readout_t))
